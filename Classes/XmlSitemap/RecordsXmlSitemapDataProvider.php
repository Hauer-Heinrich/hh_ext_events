...
namespace HauerHeinrich\HhExtEvents\XmlSitemap;
...
class RecordsXmlSitemapDataProvider extends \TYPO3\CMS\Seo\XmlSitemap\AbstractXmlSitemapDataProvider {
    ...
    public function generateItems(): void {
        ...
        $pluginsSettings = $queryBuilderPlugin->select('uid', 'pi_flexform')
            ->from('tt_content')
            ->where(
                $queryBuilderPlugin->expr()->eq('CType', $queryBuilderPlugin->createNamedParameter('hhextevents_eventlist'))
            )
        ...
        if (!empty($pluginsSettings)) {
            foreach ($pluginsSettings as $plugin) {
                ...
                $pluginSettings = $flexformArray['data']['main']['lDEF'];
                $pid = $pluginSettings['settings.startingpoint']['vDEF'];
                $pageId = $pluginSettings['settings.detailPage']['vDEF'];
